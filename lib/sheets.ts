...
const DEFAULT_TAB = "Registros Eventos";
...
export async function appendEventRegistrationRow(
  data: EventRegistrationInput,
  meta: { pageOrigin: string }
): Promise<SheetsResult> {
  const spreadsheetId = process.env.GOOGLE_SHEETS_SPREADSHEET_ID;
  const tab = process.env.GOOGLE_SHEETS_EVENTOS_TAB || DEFAULT_TAB;
